...
[Homebrew tap](https://github.com/yutat23/homebrew-tap) からインストールできます。Homebrew 6 ではサードパーティ tap を先に trust する必要があります。
...
| `ctrl+u` / `ctrl+d` | 1 ページ上 / 下へ移動（検索中も可） |
| ヘッダークリック | その列でソート（もう一度で降順） |
| `s` / `S` | ソート列を切り替え / 昇降順 |
| `p` | PID が不明な行を隠す（切り替え） |
| `4` | IPv4 の行だけ表示（もう一度で解除） |
| `6` | IPv6 の行だけ表示（もう一度で解除） |
| `y` | 選択中の `addr:port` をコピー |
| `a` | 2 秒ごとの自動更新（切り替え） |
| `enter` / `space` / `▸` をクリック | グループ内のソケットを展開・折りたたみ |
| `h` / `l` | 折りたたみ / 展開 |
| `esc` / `ctrl+c` | 検索をクリア |
| `r` | 今すぐ再読み込み |
| `x` | 選択中プロセスを終了（確認あり） |
...
同じプロセスまたは Docker コンテナで、プロトコルとホスト側ポートが同じソケット（例えば IPv4 + IPv6）は、最初 `▸` と `+N` で折りたたみます。`enter` で展開すると小さなツリーになります。先頭は `▾`、最後の子は `└─`、それ以外の子は `├─` です。ソケットが 2 つのときは子は `└─` だけ、3 つ以上のときは `├─`、…、`└─` と並びます。ポートやプロトコルが異なるものは別グループになり、Docker は PID の代わりにコンテナ ID でグループを区別します。

`4` と `6` のフィルターはどちらか一方を選べます。もう一方のキーで切り替わり、同じキーをもう一度押すと両方の表示に戻ります。
...
Docker の公開ポートにも対応しています。`/var/run/docker.sock` 経由でローカルの Docker Engine API から公開ポート情報を取得し、OS から取得した一覧に追加します。実行ユーザーには、このソケットへのアクセス権が必要です。Docker がない場合や、ソケットに接続できない場合、API が失敗した場合も、通常のプロセス一覧は表示します。公開ポート情報は、コンテナがホスト側のソケットを所有していることを示すものではありません。

Docker の行は `PROCESS` に `docker:grafana` のようなコンテナ名を表示し、TUI の詳細欄にはコンテナ側のポートとプロトコル、コンテナ ID を表示します。PID を持たないため、`p` / `-p` で非表示になり、`x` / `-k` でコンテナを停止することはできません。
